    ...
    <div style={{ background: 'var(--paper)', minHeight: '100vh' }} className="flex items-center justify-center p-6">
      <div className="w-full max-w-sm animate-fade-in">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: 'var(--ink-soft)' }}>{uz.register.fullNameLabel}</label>
            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} required className="field" placeholder={uz.register.fullNamePlaceholder} id="reg-name" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: 'var(--ink-soft)' }}>{uz.register.emailLabel}</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required className="field" placeholder={uz.register.emailPlaceholder} id="reg-email" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: 'var(--ink-soft)' }}>{uz.register.passwordLabel} <span className="eyebrow">{uz.register.passwordHint}</span></label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={8} className="field" placeholder={uz.register.passwordPlaceholder} id="reg-password" />
          </div>
